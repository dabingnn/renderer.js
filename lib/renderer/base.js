...
let _type2uniformValue = {
  ...
  [enums.PARAM_INT4]: {
    ...
    convertArray: function (value) {
      let result =  _int64_pool.alloc();
      for(let i = 0; i < value.length; ++i) {
        result[4 * i] = value[i].x;
        result[4 * i + 1] = value[i].y;
        result[4 * i + 2] = value[i].z;
        result[4 * i + 3] = value[i].w;
      }
      return result;
    },
    size: 4,
  ...
  [enums.PARAM_COLOR4]: {
    ...
    convertArray: function (value) {
      let result =  _float64_pool.alloc();
      for(let i = 0; i < value.length; ++i) {
        result[4 * i] = value[i].r;
        result[4 * i + 1] = value[i].g;
        result[4 * i + 2] = value[i].b;
        result[4 * i + 3] = value[i].a;
      }
      return result;
    },
    size: 4,
  ...
  [enums.PARAM_MAT2]: {
    ...
    convertArray: function (value) {
      let result =  _float64_pool.alloc();
      for(let i = 0; i < value.length; ++i) {
        result[4 * i] = value[i].m00;
        result[4 * i + 1] = value[i].m01;
        result[4 * i + 2] = value[i].m02;
        result[4 * i + 3] = value[i].m03;
      }
      return result;
    },
    size: 4
  ...
  [enums.PARAM_MAT4]: {
    ...
    convertArray: function (value) {
      let result =  _float64_pool.alloc();
      for(let i = 0; i < value.length; ++i) {
        result[16 * i] = value[i].m00;
        result[16 * i + 1] = value[i].m01;
        result[16 * i + 2] = value[i].m02;
        result[16 * i + 3] = value[i].m03;
        result[16 * i + 4] = value[i].m04;
        result[16 * i + 5] = value[i].m05;
        result[16 * i + 6] = value[i].m06;
        result[16 * i + 7] = value[i].m07;
        result[16 * i + 8] = value[i].m08;
        result[16 * i + 9] = value[i].m09;
        result[16 * i + 10] = value[i].m10;
        result[16 * i + 11] = value[i].m11;
        result[16 * i + 12] = value[i].m12;
        result[16 * i + 13] = value[i].m13;
        result[16 * i + 14] = value[i].m14;
        result[16 * i + 15] = value[i].m15;
      }
      return result;
    },
    size: 16
  ...
  _draw(item) {
    ...
    for (let i = 0; i < technique._parameters.length; ++i) {
      ...
      if (
        ...
        let convertedValue;
        if(prop.size !== undefined) {
          if(prop.type === enums.PARAM_COLOR3 || prop.type === enums.PARAM_FLOAT3
          || prop.type === enums.PARAM_INT3 || prop.type === enums.PARAM_MAT3) {
            console.error('Uniform array of color3/int3/float3/mat3 can not be supportted!');
            continue;
          }
          if(prop.size * convertFn.size > 64) {
            console.error('Uniform array is too long!');
            continue;
          }
          convertedValue = convertFn.convertArray(param);
        } else {
          convertedValue = convertFn.convertFunc(param);
        }
